...
function App() {
  ...
  const calcSpcStats = (machineId) => {
    ...
    const std = Math.sqrt(variance) || 0.0001; // 防除以 0

    // 3. 定義管制界限與目標值 (根據機台感測器不同)
    ...
    // 4. 計算製程能力指標 Cpk = min((USL - mean)/(3*std), (mean - LSL)/(3*std))
  ...
  // =========================================================================
  // 📐 SVG 座標轉換器 (將物理數值對應到 450x110 像素的 SPC 圖表空間)
  // =========================================================================
            ...
            {Object.keys(telemetry).map(key => {
              ...
              return (
                ...
                >
                  ...
                  {/* 機台中文名稱 */}
                  <div style={{fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.75rem'}}>{node.name}</div>
                  ...
                  <div style={{
                    ...
                    <div style={{display: 'flex', justifyContent: 'space-between'}}>
                      ...
                      <span style={{color: '#38bdf8', fontWeight: '600'}}>{node.sensor1} <span style={{fontSize: '0.65rem', color: 'var(--text-muted)'}}>{node.s1Unit}</span></span>
                    </div>
                    <div style={{display: 'flex', justifyContent: 'space-between'}}>
                      <span style={{color: 'var(--text-muted)'}}>{node.s2Name}:</span>
                      <span style={{color: '#fbbf24', fontWeight: '600'}}>{node.sensor2} <span style={{fontSize: '0.65rem', color: 'var(--text-muted)'}}>{node.s2Unit}</span></span>
                    </div>
                  </div>

                  {/* AI 預估健康度條 */}
                  <div>
                    <div style={{display: 'flex', justifyContext: 'space-between', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.25rem'}}>
                      <span>EES 診斷健康度</span>
