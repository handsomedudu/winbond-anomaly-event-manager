...
### 使用者情境定位
本系統的主要使用者是 **Fab 現場值班工程師 / EAP-EES Duty Engineer**。目前前端以「值班工程師 DUDU」作為模擬登入身份，首頁先呈現交接提醒、由緊急到不緊急排序的當班代辦、交班留言、班內訊息，以及可由兩台手機共同加入的 Jitsi 值班通話室。指派負責人不是任意操作，而是 DUDU 在 Ack 後依 SOP 將異常派給對應設備/製程工程師的值班調度行為。

這次修正的方向是把展示情境從「單人看板」推進成「值班協作工作台」：交班留言與班內訊息新增 Cloudflare D1 雲端資料表與 `/api/shift/*` API，前端每 3 秒同步一次，因此兩台手機開同一個部署網址時，可以分別切換成 DUDU、OP-8802、Alice Lin 或 Kevin Chang 送訊息，達到類似 LINE 群組的展示效果。語音與視訊則採用免費 Jitsi Meet 房間，避免自架 WebRTC signaling / STUN / TURN 造成部署複雜度，讓面試演示時能用兩台手機直接進同一間會議室測試。
...
* **階段一：基礎原型開發**：先完成異常事件管理的基本 CRUD、事件列表、狀態篩選、明細檢視與狀態流轉，確認題目要求的主要功能可以被操作。
* **階段二：初版測試與修正**：針對操作流程、表單防呆、狀態更新、資料持久化與手機排版進行修正，讓系統從「可以跑」調整到「比較像現場人員能用」。
* **階段三：核心功能補齊**：依照功能要求補上 Ack、Assign、Close 的 SOP 流程、負責工程師指派、處理對策填寫、附件佐證、統計資訊與 EES/Fab 情境化看板。
* **階段四：功能完成檢核**：逐項檢查題目要求是否完成，並實際透過瀏覽器、API 與資料庫狀態驗證，確認異常事件從查詢到結案可以形成完整閉環。
* **階段五：詢問 Fab 工程師友人回饋**：在初版功能可展示後，詢問周遭約 6 位身為 Fab 工程師或熟悉現場輪班情境的友人，整理他們對現場值班、交班、手機操作、異常排查與溝通協作的期待。
* **階段六：依回饋新增功能**：根據回饋新增 DUDU 值班工程師工作台、緊急代辦排序、交班留言、班內訊息、語音/視訊通話室、手機自適應，以及 AI SOP 診斷與多模態佐證流程。
...
### 6. 朋友測試後的異常訊息擴充
在邀請周遭 Fab 工程師友人測試後，收到一個很實際的回饋：原本 5 筆異常事件雖然足以展示基本 CRUD 與狀態流轉，但不足以進一步測試與 Demo。因為現場值班時通常不會只有一兩筆異常，若資料量太少，就很難看出搜尋、篩選、統計、手機版列表、Critical 優先排序與交班代辦的效果。

因此後續將異常事件擴充至 **20 筆**，並保留原本已解決的 Closed 事件不動，避免破壞既有處置紀錄。新增資料刻意涵蓋 SECS/GEM 通訊中斷、Run-to-Run 補償超限、APC 回寫異常、OHT 搬送逾時、MES recipe 下載逾時、interlock、SPC rule 觸發與 wafer map 比對失敗等情境，讓系統更適合展示 Fab 值班工程師同時面對多筆異常時的排序、指派與協作流程。

### 7. 手機版焦點與交班大盤修正
後續測試時也收到另一個重要回饋：手機版雖然所有功能都能使用，但全部內容堆在同一條長頁面中，滑動距離太長，使用者容易不知道當下應該先看哪一段。尤其統計卡片原本放在較下方，看到 Total / Pending / Ack / Assign / Closed 數字時，若點擊後沒有明確跳到列表，就會變成只是在展示數字，無法支援現場交班時的快速聚焦。

因此將統計卡片改為首頁最上方的 **交班大盤 Case 數量統整**，並讓每張卡片都成為可操作入口：點擊 Pending、Ack、Assign、Closed 會自動套用狀態篩選並滑到異常事件列表。手機版另外加入「值班 / 事件 / AI / 協作」快速導覽，讓 Demo 時可以用更短路徑切換重點區塊，不需要一路滑到底才找到要展示的功能。
